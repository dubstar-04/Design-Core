...
describe('Test Insert.draw', () => {
  /**
   * Create a minimal mock renderer with a jest spy for applyTransform.
  ...
  test('draw() calls renderer.applyTransform with the insert point coordinates', () => {
    ...
    ins.draw(ctx);
    expect(ctx.applyTransform).toHaveBeenCalledWith(expect.objectContaining({ x: 50, y: 75 }));
  });

  test('draw() calls renderer.applyTransform with rotation converted to radians', () => {
    const ins = new Insert({ points: [new Point(0, 0)], rotation: 90 });
    const ctx = makeMockCtx();
    ins.draw(ctx);
    expect(ctx.applyTransform).toHaveBeenCalledWith(expect.objectContaining({ rotation: Utils.degrees2radians(90) }));
  });

  test('draw() calls renderer.applyTransform with rotation 0 when rotation is 0', () => {
    const ins = new Insert({ points: [new Point(0, 0)] });
    const ctx = makeMockCtx();
    ins.draw(ctx);
    expect(ctx.applyTransform).toHaveBeenCalledWith(expect.objectContaining({ rotation: 0 }));
  });

  test('draw() calls renderer.applyTransform exactly once', () => {
