...
test('constructor sets default properties', () => {
  const dim = new DiametricDimension();
  expect(dim).toBeInstanceOf(DiametricDimension);
...
test('getPointsFromSelection returns correct sequenced points', () => {
  ...
  const result = DiametricDimension.getPointsFromSelection([item], textPos);
...
test('DiametricDimension.preview runs without error and calls createTempItem', () => {
  ...
  const dim1 = new DiametricDimension();
  dim1.points = points;
  ...
  const dim2 = new DiametricDimension();
  dim2.points = points;
...
test('Test DiametricDimension.dxf', () => {
  const points = [];
...
`;
  ...
  const newDimension = new DiametricDimension({ handle: dimension.getProperty('handle'), points: dimension.points, ...dimension });
  file = new File();
