...
test('Test intersectSegmentSegment - both segments extended (ray-ray equivalent)', () => {
  // Perpendicular lines meeting at origin
  const r1 = Intersection.intersectSegmentSegment(
      new Point(-5, 0), new Point(5, 0),
      new Point(0, -5), new Point(0, 5),
      true, true,
  );
  expect(r1.points.length).toBe(1);
  expect(r1.points[0].x).toBeCloseTo(0);
  expect(r1.points[0].y).toBeCloseTo(0);

  // Non-perpendicular: lines y=1 and x=1 meeting at (1, 1)
  const r2 = Intersection.intersectSegmentSegment(
      new Point(0, 1), new Point(10, 1),
      new Point(1, 0), new Point(1, 10),
      true, true,
  );
  expect(r2.points.length).toBe(1);
  expect(r2.points[0].x).toBeCloseTo(1);
  expect(r2.points[0].y).toBeCloseTo(1);

  // 45-degree lines crossing at (5, 5)
  const r3 = Intersection.intersectSegmentSegment(
      new Point(0, 0), new Point(10, 10),
      new Point(10, 0), new Point(0, 10),
      true, true,
  );
  expect(r3.points.length).toBe(1);
  expect(r3.points[0].x).toBeCloseTo(5);
  expect(r3.points[0].y).toBeCloseTo(5);

  // Parallel lines — should return no points
  const r4 = Intersection.intersectSegmentSegment(
      new Point(0, 0), new Point(10, 0),
      new Point(0, 1), new Point(10, 1),
      true, true,
  );
  expect(r4.points.length).toBe(0);

  // Disjoint segments that only intersect when both extended
  const r5 = Intersection.intersectSegmentSegment(
      new Point(0, 0), new Point(2, 0),
      new Point(5, -2), new Point(5, -1),
      true, true,
  );
  expect(r5.points.length).toBe(1);
  expect(r5.points[0].x).toBeCloseTo(5);
  expect(r5.points[0].y).toBeCloseTo(0);
});
