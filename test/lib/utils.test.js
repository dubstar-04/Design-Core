...
test('Test Utils.sortPointsOnArc mutates array ordered by angular position', () => {
  ...
  Utils.sortPointsOnArc(pts, arc.points[1], arc.points[2], arc.points[0], arc.getProperty('direction'));
  ...
  Utils.sortPointsOnArc(pts, cwArc.points[1], cwArc.points[2], cwArc.points[0], cwArc.getProperty('direction'));
  // Expect order: (-10,0), (-7.1, 7.1), (10,10), (7.1, 7.1), (10,0)
  expect(pts[0].x).toBe(-10);
  expect(pts[0].y).toBe(0);
