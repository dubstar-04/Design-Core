...
export class Chamfer extends ChamferFilletBase {
  ...
  preview() {
    ...
    const geo = this.#computeChamfer(this.firstPick, tempSecond, intersectionPoint);
    if (!geo) return;
    ...
    DesignCore.Scene.previewEntities.add(DesignCore.CommandManager.createNew('Line', {
      points: [this.firstPick.lineKeptEnd(intersectionPoint), geo.firstChamferPoint],
      ...Utils.cloneProperties(this.firstPick.entity),
    }));
    DesignCore.Scene.previewEntities.add(DesignCore.CommandManager.createNew('Line', {
      points: [tempSecond.lineKeptEnd(intersectionPoint), geo.secondChamferPoint],
