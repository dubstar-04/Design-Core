...
export class PropertyManager {
  ...
  setItemProperties(property, newPropertyValue) {
    ...
    for (let i = 0; i < this.core.scene.selecting.selectionSet.length; i++) {
      // check if the item has the selected property
      if (!this.core.scene.items[this.core.scene.selecting.selectionSet[i]].hasOwnProperty(property)) {
        continue;
      }

      if (typeof(this.core.scene.items[this.core.scene.selecting.selectionSet[i]][property]) !== typeof(newPropertyValue)) {
  ...
  getItemTypes() {
    ...
    if (this.core.scene.selecting.selectionSet.length > 0) {
      for (let i = 0; i < this.core.scene.selecting.selectionSet.length; i++) {
        const itemType = this.core.scene.items[this.core.scene.selecting.selectionSet[i]].type;
        // console.log(this.core.scene.items[this.core.scene.selecting.selectionSet[i]].type);
  ...
  getItemProperties(itemType) {
    ...
    if (itemType === undefined || itemType === null || this.core.scene.selecting.selectionSet.length <= 0) {
      return;
    }
    ...
    this.core.scene.selecting.selectionSet.forEach((index) => {
  ...
  getItemPropertyValue(itemType, property) {
    ...
    if (this.core.scene.selecting.selectionSet.length > 0) {
      for (let i = 0; i < this.core.scene.selecting.selectionSet.length; i++) {
        if (this.core.scene.items[this.core.scene.selecting.selectionSet[i]].type === itemType || itemType === 'All') {
          const prop = this.core.scene.items[this.core.scene.selecting.selectionSet[i]][property];
