...
setPrompt(prompt) {
    this.prompt = this.scene.activeCommand.type + ": " + prompt;
	this.command = "";
    this.update();
}

setPromptText(promptString) {
    this.prompt = this.scene.activeCommand.type + ": " + promptString;
	this.command = "";
    this.update();
}
...
handleKeys(e) {
    ...
    switch (charCode) {
        ...
        case 27: // Escape
            var data = [];
            designEngine.sceneControl("RightClick", []);
            break;
        case 32: // space
            this.enterPressed(e);
            break;
...
deletePressed(event) {
	...
	designEngine.sceneControl('Enter', ['E'] )
...
enterPressed(event) {
    ...
    if (this.cmdLine.value.length > this.prompt.length) {
        ...
        //console.log(data[0])
        designEngine.sceneControl("Enter", data);
    } else {
        var data = [];
        designEngine.sceneControl("Enter", data);
    }
