...
export class Scene {
    ...
    selectionSetChanged() {
        //TODO: Move Properties to own class
        //getProperties()
    }
    ...
    getSceneExtents() {
        var width = this.core.canvas.width / this.core.canvas.scale;
        var height = this.core.canvas.height / this.core.canvas.scale;

        var xmin = -this.core.canvas.panTotal.x / this.core.canvas.scale;
        var xmax = xmin + width;
        var ymax = -this.core.canvas.panTotal.y / this.core.canvas.scale;
    ...
    addItemToBlock(type, data, name) {
        ...
        if (this.items[this.items.length - 1].name === name) {
            ...
            if (type === "Insert") {
                ...
                var item = this.core.commandManager.createNew(type, data);
    ...
    findClosestItem() {
        ...
        for (var i = 0; i < this.items.length; i++) {
            var distance = this.items[i].closestPoint(this.core.mouse.pointOnScene())[1]; //ClosestPoint()[1] returns a distance to the closest point
    ...
    selectClosestItem(data) {
        ...
        if (closestItem !== undefined) {
            if (this.selectionSet.indexOf(closestItem) === -1) { // only store selections once
                var copyofitem = Utils.cloneObject(this.core, this.items[closestItem]);
...
selecting() {
    ...
    if (this.core.mouse.buttonOneDown) {
        var selectionPoints = [];
        selectionPoints.push(this.core.mouse.mouseDownPoint);
        selectionPoints.push(this.core.mouse.pointOnScene());

        var selectColour;

			if (this.core.mouse.pointOnScene().y > this.core.mouse.mouseDownPoint.y) {
				//Draw a rectangle on screen
				selectColour = "#FF0000"
			} else if (this.core.mouse.pointOnScene().y < this.core.mouse.mouseDownPoint.y) {
				//Draw a rectangle on screen
				selectColour = "#0000FF"
			}
        ...
        var tempItem = this.core.commandManager.createNew("FilledRectangle", data); // Create a new item, send it the tempPoints array
        ...
        this.core.mouse.mouseDownPoint
        this.core.mouse.pointOnScene()

        var xmin = Math.min(this.core.mouse.mouseDownPoint.x, this.core.mouse.pointOnScene().x);
        var xmax = Math.max(this.core.mouse.mouseDownPoint.x, this.core.mouse.pointOnScene().x);
        var ymin = Math.min(this.core.mouse.mouseDownPoint.y, this.core.mouse.pointOnScene().y);
        var ymax = Math.max(this.core.mouse.mouseDownPoint.y, this.core.mouse.pointOnScene().y);
        ...
        for (var i = 0; i < this.items.length; i++) {
            if (this.core.mouse.pointOnScene().y > this.core.mouse.mouseDownPoint.y) {
                ...
                if (this.items[i].touched(selection_extremes, this.core) || this.items[i].within(selection_extremes, this.core)) {
                    ...
                    if (this.selectionSet.indexOf(i) === -1) { // only store selections once
                        var copyofitem = Utils.cloneObject(this.core, this.items[i]);
                        copyofitem.colour = this.core.settings.selectedItemsColour.toString();
                        copyofitem.lineWidth = copyofitem.lineWidth * 2;
                ...
                if (this.items[i].within(selection_extremes, this.core)) {
...
addHelperGeometry(type, points, colour) {
    ...
    var helper = this.core.commandManager.createNew(type, data);
...
addSnapPoint(snapPoint) {
    ...
    var CentrePoint = new Point(snapPoint.x, snapPoint.y);
    var radiusPoint = new Point(snapPoint.x, snapPoint.y + (5 / this.core.canvas.scale));
...
mouseMoved() {
    ...
    if (this.activeCommand !== undefined && this.activeCommand.family === "Geometry" || this.selectionAccepted === true && this.activeCommand.movement !== "Modify") {
        var snapPoint = Snapping.getSnapPoint(this);
        if (snapPoint) {
    ...
    */
    ...
    if (this.points.length !== 0) {
        ...
        if (this.core.settings.polar) {
            ...
            var polarSnap = Snapping.polarSnap(this.lastSelectedPoint(), this.core);
            if (polarSnap) {
                this.core.mouse.setPos(polarSnap);
            }
        } else if (this.core.settings.ortho) {
            //if ortho is enabled - get the nearest ortho point
            var orthoSnap = Snapping.orthoSnap(this.lastSelectedPoint(), this.core);
            if (orthoSnap) {
                this.core.mouse.setPos(orthoSnap);
            }
        }
