...
Scene.prototype.selectionSetChanged = function() {
    getProperties()
}

Scene.prototype.centreVPORT = function(centre, width, height) {
...
Scene.prototype.getSceneExtents = function() {
...
Scene.prototype.saveRequired = function() {
    saved = false; //Changes have occured. A save may be required.
}

// add item to block
//TODO: fix this mess
Scene.prototype.addItemToBlock = function(type, data, name) {
...
Scene.prototype.sceneLinkBlockData = function() {
    ...
    for (idx = 0; idx < this.items.length; idx++) {
        ...
        if (this.items[idx].type === 'Dimension') {
            ...
            block = blocks.filter(item => item.name === this.items[idx].blockName)
            if (block) {
...
Scene.prototype.addToScene = function(type, data, end, index) {
    ...
    if (this.activeCommand && this.activeCommand.family === "Geometry" && !type) {
        ...
        item = new window[type](data); // Create a new item, send it the points array
    }
...
Scene.prototype.writeStatusMessage = function(label, message) {
    label.statusText = message
}

Scene.prototype.findClosestItem = function() {
...
Scene.prototype.selectClosestItem = function(data) {
    ...
    if (closestItem !== undefined) {
        ...
        if (this.selectionSet.indexOf(closestItem) === -1) { // only store selections once
            ...
            var copyofitem = cloneObject(this.items[closestItem]);
...
Scene.prototype.snapping = function() {
    ////////// Object Snapping //////////
    var snaps = new Array();
...
Scene.prototype.selecting = function(coordinates, SelectColour) {
            ...
            if (coordinates[3] > coordinates[1]) {
                ...
                if (this.items[i].touched(selection_extremes) || this.items[i].within(selection_extremes)) {
                    ...
                    if (this.selectionSet.indexOf(i) === -1) { // only store selections once
                        var copyofitem = cloneObject(this.items[i]);
                        copyofitem.colour = settings.selectedItemsColour.toString();
                        copyofitem.lineWidth = copyofitem.lineWidth * 2;
...
Scene.prototype.addHelperGeometry = function(type, points, colour) {
...
Scene.prototype.polarSnap = function(previousPoint) {
...
Scene.prototype.orthoSnap = function(previousPoint) {
...
Scene.prototype.writeCoordinates = function(label, coordinates) {
    this.tempItems = [];
    this.tempPoints = [];
    ...
    if (this.points.length !== 0) {
        ...
        if (this.activeCommand !== undefined && this.activeCommand.showPreview && this.activeCommand.family === "Tools" && this.selectionAccepted) {
            console.log("preview")
            this.activeCommand.preview(this.tempPoints, this.selectedItems, this.items);
            canvas.requestPaint();
        }
