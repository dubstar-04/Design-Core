...
//this.core.canvas; // Canvas to which the items should be drawn
this.inputPromptBox; // inputPrompt to get user input //TODO: can this be removed?
//this.core.mouse = new Point();                            // store the mouse coordinates.
//this.core.mouse_delta = new Point(0,0);
...
openFile(file){
    FileIO.openFile(this.core, file)
}

saveFile(){
    return FileIO.saveDxf(this.core);
}
...
getSceneExtents() {
    ...
    var size = this.core.canvas.canvasSize();
    var width = size.width / this.core.canvas.scale;
    var height = size.height / this.core.canvas.scale;

    var xmin = -this.core.canvas.panX / this.core.canvas.scale;
    var xmax = xmin + width;
    var ymax = -this.core.canvas.panY / this.core.canvas.scale;
...
findClosestItem() {
    ...
    var mousePoint = new Point( this.core.mouse.x,this.core.mouse.y);
    var closestItem;
...
snapping() {
    ////////// Object Snapping //////////
    var snaps = new Array();
    var delta = 25 / this.core.canvas.scale; // find a more suitable starting value
    //var itemIndex = 0;
    var mousePoint = new Point( this.core.mouse.x,this.core.mouse.y);
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        if (itemSnaps) {
            for (var j = 0; j < itemSnaps.length; j++) {
                var length = Utils.distBetweenPoints(itemSnaps[j].x, itemSnaps[j].y,this.core.mouse.x,this.core.mouse.y)
                if (length < delta) {
                    delta = length;
                    //var closestItem = i;
                    ...
                    var radiusPoint = new Point(itemSnaps[j].x, itemSnaps[j].y + (5 / this.core.canvas.scale));
                   ...
                   this.core.mouse.x = itemSnaps[j].x;
                   this.core.mouse.y = itemSnaps[j].y;
...
polarSnap(previousPoint) {
    ...
    var angleDelta = 3;
    var diff = Utils.radians2degrees(previousPoint.angle(this.core.mouse)) - (settings.polarAngle * Math.round(Utils.radians2degrees(previousPoint.angle(this.core.mouse)) / settings.polarAngle))

    if (Math.abs(diff) < angleDelta) {
        //console.log("scene.js - polarSnap - Diff:", diff, " canvas size", canvas.canvasSize)
        var mousept = new Point( this.core.mouse.x,this.core.mouse.y);
        //commandManager.createNew("Point", data);
        //new Point(mouse.x,this.core.mouse.y);
        mousept = mousept.rotate(previousPoint, Utils.degrees2radians(-diff))

       this.core.mouse.setX(mousept.x);
       this.core.mouse.setY(mousept.y)
...
orthoSnap(previousPoint) {
    ...
    var x =this.core.mouse.x - previousPoint.x
    var y =this.core.mouse.y - previousPoint.y

    if (Math.abs(x) > Math.abs(y)) {
       this.core.mouse.setY(previousPoint.y)
    } else {
       this.core.mouse.setX(previousPoint.x)
    }
