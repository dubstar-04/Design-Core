...
mouseDown(ev) {
	...
	switch (button) {
		case 0: //left button
			var data = [];
			designEngine.sceneControl("LeftClick", data);
			 //this.designEngine.mouse.downX =this.designEngine.mouse.x;
			 //this.designEngine.mouse.downY =this.designEngine.mouse.y;
			 this.designEngine.scene.selectingActive = true;
			break;
		...
		case 2: //right button
			...
			//var data = true;
			//designEngine.sceneControl("RightClick", data);
			break;
			//default:
			//    default code block
	}
	...
	switch (button) {
		case 0: //left button
			// console.log("left click")
			 this.designEngine.scene.selectingActive = false;
			 this.designEngine.scene.selecting([ this.designEngine.mouse.downX, this.designEngine.mouse.downY, this.designEngine.mouse.x, this.designEngine.mouse.y], "");
			break;
		case 1: //middle button
...
pan() {
	if (this.panning) {
		var deltaX = this.designEngine.mouse.canvasX - this.designEngine.mouse.lastX;
		var deltaY = ( this.designEngine.mouse.canvasY - this.designEngine.mouse.lastY);
...
selecting() {
	...
	if ( this.designEngine.scene.selectingActive) {
		if (Utils.distBetweenPoints( this.designEngine.mouse.downX, this.designEngine.mouse.downY, this.designEngine.mouse.x, this.designEngine.mouse.y) > 5) {

			// console.log("Selecting..........")

			if ( this.designEngine.mouse.y > this.designEngine.mouse.downY) {
				//Draw a rectangle on screen
				 this.designEngine.scene.selecting([ this.designEngine.mouse.downX, this.designEngine.mouse.downY, this.designEngine.mouse.x, this.designEngine.mouse.y], "#FF0000");
			} else if ( this.designEngine.mouse.y < this.designEngine.mouse.downY) {
				//Draw a rectangle on screen
				 this.designEngine.scene.selecting([ this.designEngine.mouse.downX, this.designEngine.mouse.downY, this.designEngine.mouse.x, this.designEngine.mouse.y], "#0000FF");
			}
		}
	}
}
...
zoom(scale) {
	...
	var x = ( this.designEngine.mouse.canvasX - this.panX) / this.scale;
	var y = ( this.designEngine.mouse.canvasY - this.panY) / this.scale;
...
zoomExtents() {
	...
	if ( this.designEngine.scene.items.length) {
...
getExtents() {
	...
	for (var i = 0; i <  this.designEngine.scene.items.length; i++) {
		var extremes =  this.designEngine.scene.items[i].extremes();
...
requestPaint() {
	...
	var numOfEntities =  this.designEngine.scene.items.length
	var i = 0,
		j = 0,
		k = 0;
	...
	for (i; i < numOfEntities; i++) {
		 this.designEngine.scene.items[i].draw(this.context, this.scale, this.designEngine);
	}

	for (j; j <  this.designEngine.scene.tempItems.length; j++) {
		 this.designEngine.scene.tempItems[j].draw(this.context, this.scale, this.designEngine);
	}

	for (k; k <  this.designEngine.scene.selectedItems.length; k++) {
		 this.designEngine.scene.selectedItems[k].draw(this.context, this.scale, this.designEngine);
	}
}
...
paintGrid() {
	...
	var extents =  this.designEngine.scene.getSceneExtents();
