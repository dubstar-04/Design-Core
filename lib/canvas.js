...
export class Canvas {
	...
	zoomExtents() {
		...
		var extents = this.core.scene.getExtents()
    if(extents){
      // calculate the center of all items
	    var selectionCenter = new Point(extents.xmin + ((extents.xmax - extents.xmin) / 2), extents.ymin + ((extents.ymax - extents.ymin) / 2));
      ...
      var translateDelta = this.core.mouse.transformToScene(screenCenter).subtract(selectionCenter)
      // calculate the scale required to fill the screen
	    var targetScale = Math.min((this.width / (extents.xmax - extents.xmin)), (this.height / (extents.ymax - extents.ymin)));
	    // calculate the scale delta required to fill 90% of the screen
	    var scaleDelta = targetScale / this.getScale() * 0.9;
      // apply scale
	...
	paint(context, width, height) {
		...
		try {// HTML
			...
			var rgbColour = Colours.hexToScaledRGB(this.core.settings.canvasBackgroundColour)
			...
			var scaled = new Point(width, height)
			var sc = this.core.mouse.transformToScene(scaled);
	...
	paintGrid(context) {
		...
		try { // HTML Canvas
			...
			var rgbColour = Colours.hexToScaledRGB(this.core.settings.gridColour)
			context.setSourceRGB(rgbColour.r, rgbColour.g, rgbColour.b);
		}

		var extents = this.getSceneOffset();
    ...
    getSceneOffset() {
        ...
        var width = this.width / this.getScale();
        var height = this.height / this.getScale();

        var xmin = -this.matrix.e / this.getScale();
        var xmax = xmin + width;
        var ymin = -height + this.matrix.f / this.getScale();
