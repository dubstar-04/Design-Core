...
Canvas.prototype.mousedown = function (ev) {
	...
	switch (button) {
		case 0: //left button
			...
			mouse.downX = mouse.x;
			mouse.downY = mouse.y;
			this.scene.selectingActive = true;
			break;
...
Canvas.prototype.mouseup = function (ev) {
	...
	switch (button) {
		case 0: //left button
			...
			this.scene.selectingActive = false;
			this.scene.selecting([mouse.downX, mouse.downY, mouse.x, mouse.y], "");
			break;
		case 1: //middle button
	...
	if (this.scene.selectingActive) {
		if (distBetweenPoints(mouse.downX, mouse.downY, mouse.x, mouse.y) > 5) {
			...
			if (mouse.y > mouse.downY) {
				...
				this.scene.selecting([mouse.downX, mouse.downY, mouse.x, mouse.y], "#FF0000");
			} else if (mouse.y < mouse.downY) {
				//Draw a rectangle on screen
				this.scene.selecting([mouse.downX, mouse.downY, mouse.x, mouse.y], "#0000FF");
			}
		}
	}
}
...
Canvas.prototype.getExtents = function () {
	...
	for (var i = 0; i < this.scene.items.length; i++) {
		var extremes = this.scene.items[i].extremes();
...
Canvas.prototype.requestPaint = function () {
	...
	var numOfEntities = this.scene.items.length
	var i = 0,
		j = 0,
		k = 0;
	...
	for (i; i < numOfEntities; i++) {
		this.scene.items[i].draw(this.context, this.scale);
	}

	for (j; j < this.scene.tempItems.length; j++) {
		this.scene.tempItems[j].draw(this.context, this.scale);
	}

	for (k; k < this.scene.selectedItems.length; k++) {
		this.scene.selectedItems[k].draw(this.context, this.scale);
	}
}
