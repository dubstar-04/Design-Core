...
export class Mouse {
	...
	transformToScene(point){
		...
		//return new Point(((point.x - this.core.canvas.panTotal.x) / this.core.canvas.scale),((point.y - this.core.canvas.panTotal.y) / this.core.canvas.scale))
		//return new Point(point.x / this.core.canvas.scale - this.core.canvas.panTotal.x, point.y / this.core.canvas.scale - this.core.canvas.panTotal.y)
		...
		var xpos = ((point.x - xposscale) / this.core.canvas.scale) - this.core.canvas.panTotal.x 
		var ypos = ((point.y - yposscale) / this.core.canvas.scale) - this.core.canvas.panTotal.y
	...
	mouseMoved(x, y) {
		...
		this.x = x; //((x - this.core.canvas.panX) / this.core.canvas.scale);
		this.y = y; //((y - this.core.canvas.panY) / this.core.canvas.scale);

		this.core.scene.mouseMoved();

		//TODO: mouse should store which buttons are down and the canvas and scene can decide of there panning and selecting
