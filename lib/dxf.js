...
static addElementToScene(type, data) {
    ...
    if (this.blockName !== "" || type == "Insert") {
        ...
        this.scene.addItemToBlock(type, data, name);
    } else {
        this.scene.addToScene(type, data)
    }
}
    ...
    while (this.lineNum < this.lines.length) {
        ...
        switch (n) {
            case 0:
                ...
                if (name) {
                    this.scene.designEngine.LM.addLayer(layer)
                }
                return;
    ...
    while (this.lineNum < this.lines.length) {
        ...
        switch (n) {
            ...
            case 50:
                // start angle
                this.getDXFLine();
                start_angle = Utils.degrees2radians(Number(this.line));
                //debugLog("start angle: " + this.line);
                break;
            case 51:
                // end angle
                this.getDXFLine();
                end_angle = Utils.degrees2radians(Number(this.line));
        ...
        switch (n) {
            case 0:
                ...
                if (name !== "") {
                    ...
                    this.scene.designEngine.SM.addStyle(style)
                }
    ...
    while (this.lineNum < this.lines.length) {
        ...
        switch (n) {
            case 0:
                ...
                if (name !== "") {
                    this.scene.designEngine.DSM.addStyle(dimstyle)
                }
...
static writeDXF(scene){
    ...
    data = data.concat(
            ...
            "\n$CLAYER",
            "\n8",
            "\n" + this.scene.designEngine.LM.getCLayer(),
            "\n0",
            "\nENDSEC"
        )
    ...
    data = data.concat(
        ...
        "\nLAYER",
        "\n70",
        "\n" + this.scene.designEngine.LM.layerCount())

    for (var i = 0; i < this.scene.designEngine.LM.layerCount(); i++) {
        data = data.concat("\n", this.scene.designEngine.LM.getLayerByIndex(i).dxf())
    }
    ...
    data = data.concat(
        ...
        "\nSTYLE",
        "\n70",
        "\n" + this.scene.designEngine.SM.styleCount())

    for (var i = 0; i < this.scene.designEngine.SM.styleCount(); i++) {
        data = data.concat("\n", this.scene.designEngine.SM.getStyleByIndex(i).dxf())
    }
    ...
    data = data.concat(
        ...
        "\nDIMSTYLE",
        "\n70",
        "\n" + this.scene.designEngine.DSM.styleCount())

    for (var i = 0; i < this.scene.designEngine.DSM.styleCount(); i++) {
        data = data.concat("\n", this.scene.designEngine.DSM.getStyleByIndex(i).dxf())
    }
    ...
    var extents = this.scene.getSceneExtents()
