...
export class Snapping {
    ...
    static getSnapPoint(scene) {
        ...
        for (var i = 0; i < scene.items.length; i++) {
            ...
            var itemSnaps = scene.items[i].snaps(scene.core.mouse.currentPoint(), delta, scene.core) // get an array of snap point from the item
    ...
    static polarSnap(previousPoint, core) {
        ...
        var snapPoint;
        var angleDelta = 3;
        var diff = Utils.radians2degrees(previousPoint.angle(core.mouse.currentPoint())) - (core.settings.polarAngle * Math.round(Utils.radians2degrees(previousPoint.angle(core.mouse.currentPoint())) / core.settings.polarAngle))
