...
DesignEngine.prototype.sceneControl = function(action, data) {
	...
	if (typeof this.scene.activeCommand !== "undefined") {
		...
		this.initialiseItem(commandManager.getCommandFromShortcut(input));
...
DesignEngine.prototype.actionInput = function() {

	var prompt, resetBool, actionBool, validInput;

	[prompt, resetBool, actionBool, validInput] = this.scene.activeCommand.prompt(this.scene.inputArray);
	...
	if (actionBool) {
		if (this.scene.activeCommand.family === "Tools") {
			this.scene.activeCommand.action(this.scene.points, this.scene.items);
		} else {
...
DesignEngine.prototype.initialiseItem = function(item) {
	...
	this.scene.activeCommand = commandManager.createNew(item);
	//scene.activeCommand = new window[item]; // Convert the 'item' string in to a new object, Line, Circle...

};

DesignEngine.prototype.convertInputToPoint = function (input) {
	var point = new Point()
